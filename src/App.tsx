...
function App() {
  ...
  return (
    <div className={`container ${isNumbers ? 'numbers' : ''}`}>
      <Button className={`main-button ${isNumbers ? 'main-button--low' : ''}`} type="button" onClick={onButtonClick} text={isNumbers ? 'ПОКАЗАТЬ БУКВЫ' : 'ПОКАЗАТЬ ЦИФРЫ'} />
      <div className="main" />
      {(isNumbers ? numbersList : lettersList).map((letter, i) => {
        const color = getColor();
        return (
          <Balloon className="ballon-item" type={isNumbers ? 'round' : ''} key={i} letter={letter} color={color} onClick={handleSpeak} />
        );
      })}
    </div>
  );
